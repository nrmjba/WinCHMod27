    ...
    def get_file_attr(self, filename):
        ...
        if os.path.isfile(filename):#loop for files
            ...
            if self.ci.get() == 1:
                ...
                coinher = ''
            try:
                self.dir_rights = self.get_dir_rights()
                fn=os.path.normpath(filename)
                current_username = "nwea/"+win32api.GetUserName()+":"
                if self.cb1.get() == 1:
                    os.system("icacls " + fn + " /grant :r Everyone:"+obinher+coinher+self.dir_rights)
                if self.cb3.get() == 1:
                    os.system("icacls " + fn + " /grant :r "+current_username+obinher+coinher+self.dir_rights)
                if self.cb4.get() == 1:
                    os.system("icacls " + fn + " /grant :r Users:"+obinher+coinher+self.dir_rights)
                if self.cb2.get() == 1:
                    os.system("icacls " + fn + " /grant :r Administrators:"+obinher+coinher+self.dir_rights)
